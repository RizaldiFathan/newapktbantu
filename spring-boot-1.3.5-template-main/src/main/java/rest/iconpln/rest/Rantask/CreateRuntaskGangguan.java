...
import rest.iconpln.service.Rantask.ServiceCreateRuntaskGangguan;

@RestController
@RequestMapping("/runtask/")
public class CreateRuntaskGangguan {

    private org.slf4j.Logger logger = LoggerFactory.getLogger(CreateRuntaskGangguan.class
    );
    @Autowired

    private ServiceCreateRuntaskGangguan serviceCreateRuntaskGangguan;

    // CREATE RUNTASK GANGGUAN
    // GET
    @RequestMapping(
            value = "/Runtask_GG",
    ...
    @ResponseBody
    public ResponseEntity<Object> Runtask_GG(
        ...
        try {
            ...
            mOut = serviceCreateRuntaskGangguan.Runtask_GG(mIn);
            logger.info(
                    "===|  OUT FROM = "
                    + " p_nolapor : " + p_nolapor + " | "
                    + " OUT = " + mOut
    ...
    // POST
    @RequestMapping(
            value = "/POST_RUNTASK_GG",
            produces = MediaType.APPLICATION_JSON_VALUE,
            method = RequestMethod.POST
    )
    @ResponseBody
    public ResponseEntity<Object> POST_RUNTASK_GG(
        ...
        try {
            ...
            mOut = serviceCreateRuntaskGangguan.POST_RUNTASK_GG(mIn);
            logger.info(
                    "===|  OUT FROM = "
                    + " P_NOLAPORAN : " + P_NOLAPORAN + " | "
                    + " OUT = " + mOut
