...
import rest.iconpln.service.LOG.ServicePlnMobileJobRequestEvent;
...
@RestController
@RequestMapping("/log/")
public class RestPlnMobileJobRequestEvent {
    private org.slf4j.Logger logger = LoggerFactory.getLogger(RestPlnMobileJobRequestEvent.class);
    
    @Autowired
    private ServicePlnMobileJobRequestEvent servicePlnMobileJobRequestEvent;

    // GET
    @RequestMapping(
            value = "/PLN_MOBILE_JOB_REQUEST_EVENT",
            produces = MediaType.APPLICATION_JSON_VALUE,
            method = RequestMethod.POST
    )
    @ResponseBody
    public ResponseEntity<Object> PLN_MOBILE_JOB_REQUEST_EVENT(
        ...
        try {
            ...
            mOut = servicePlnMobileJobRequestEvent.PLN_MOBILE_JOB_REQUEST_EVENT(mIn);
            logger.info(
                    "===|  OUT FROM = " +
                            " p_tglawal : " + p_tglawal + " | " +
                            " p_tglakhir : " + p_tglakhir + " | " +
